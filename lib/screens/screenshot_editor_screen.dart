...
import '../dialogs/auto_detection_dialog.dart';
...
class _ScreenshotEditorScreenState
  ...
  final GlobalKey<State<EditorCanvas>> _canvasKey = GlobalKey<State<EditorCanvas>>();
  ...
  Future<ui.Image> _loadBackgroundImage(String imagePath) async {
    try {
      Uint8List bytes;

      // Check if it's a file path or asset path
      if (imagePath.startsWith('assets/') || !imagePath.contains(Platform.pathSeparator)) {
        // It's an asset path, use rootBundle
        final ByteData data = await rootBundle.load(imagePath);
        bytes = data.buffer.asUint8List();
      } else {
        // It's a file path, read from file system
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            ElevatedButton.icon(
              onPressed: _showAutoDetectionDialog,
  ...
  void _showAutoDetectionDialog() async {
    try {
      // Get current screenshot and background image
      final screenshot = await ref.read(screenshotProvider(widget.screenshotId).future);
      if (screenshot == null) {
        _showErrorSnackBar('Screenshot not found');
        return;
      }

      final backgroundImage = await _loadBackgroundImage(screenshot.originalPath);
      // Note: _loadBackgroundImage should not return null in normal cases

      // Show auto-detection dialog
      if (mounted) {
        showDialog(
          context: context,
          barrierDismissible: false,
          builder: (context) => AutoDetectionDialog(
            image: backgroundImage,
            onGuidesGenerated: _applyAutoGeneratedGuides,
            onRedactionSuggestionsGenerated: _applyRedactionSuggestions,
          ),
        );
      }
    } catch (e) {
      _showErrorSnackBar('Failed to open auto-detection: $e');
    }
  }

  void _applyAutoGeneratedGuides(List<AutoGuide> guides) {
    try {
      if (guides.isEmpty) {
        _showInfoSnackBar('No guides were generated');
        return;
      }

      // Separate vertical and horizontal guides
      final verticalGuides = <double>[];
      final horizontalGuides = <double>[];
      ...
      final uniqueVertical = verticalGuides.toSet().toList();
      final uniqueHorizontal = horizontalGuides.toSet().toList();

      // Apply guides to canvas using the canvas key
      final canvasState = _canvasKey.currentState;
      if (canvasState != null) {
        // Use proper type checking and casting to access the private state methods
        try {
          final dynamic editorState = canvasState;

          // Get existing guides
          final List<double> existingVertical = editorState.verticalGuides ?? <double>[];
          final List<double> existingHorizontal = editorState.horizontalGuides ?? <double>[];

          // Combine with new guides and remove duplicates
          final allVertical = {...existingVertical, ...uniqueVertical}.toList();
          final allHorizontal = {...existingHorizontal, ...uniqueHorizontal}.toList();

          // Apply all guides at once
          editorState.setVerticalGuides(allVertical);
          editorState.setHorizontalGuides(allHorizontal);
        } catch (e) {
          // If dynamic access fails, log the error but don't crash
          print('Failed to update guides: $e');
        }

        // Force canvas repaint to show new guides
        setState(() {});

        _showSuccessSnackBar(
          'Applied ${uniqueVertical.length} vertical and ${uniqueHorizontal.length} horizontal guides'
        );
      } else {
        _showErrorSnackBar('Canvas not ready for guide application');
      }

    } catch (e) {
      _showErrorSnackBar('Failed to apply guides: $e');
    }
  }

  void _applyRedactionSuggestions(List<RedactionSuggestion> suggestions) {
    try {
      if (suggestions.isEmpty) {
        _showInfoSnackBar('No redaction suggestions generated');
        return;
      }
      ...
      // Apply accepted redaction suggestions as layers
      for (final suggestion in suggestions) {
        if (suggestion.isAccepted) {
          final command = AddLayerCommand(
            suggestion.suggestedLayer,
            _addLayer,
            _removeLayer,
          );
          historyService.executeCommand(command);
      ...
      if (appliedCount > 0) {
        _showSuccessSnackBar('Applied $appliedCount redaction suggestions');
      } else {
        _showInfoSnackBar('No redaction suggestions were accepted');
      }

    } catch (e) {
      _showErrorSnackBar('Failed to apply redactions: $e');
    }
  }

  // Helper methods for showing different types of snackbars
